import HeliusClient from "@/utils/helius/heliusClient";
import { PublicKey } from "@solana/web3.js";

(async () => {
  console.log(`\n\x1b[1m\x1b[34m================ start ================\x1b[0m`);

  // KGOLD MAIN_NET SOL 支付账户
  // HiLexvcqG6hkTK72tMNYcvKfJLm7vxJAYyZhmAZM2WVm
  // 
  // 
  // 

  // const keypair = KeypairUtils.generateKeypair();
  // console.log("publicKey", keypair.publicKey.toBase58());
  // const hexSecretKey = Buffer.from(keypair.secretKey).toString("hex");
  // console.log("secretKey (hex)", hexSecretKey);

  const heliusClient = HeliusClient.getInstance();

  // const tokenAccounts = await heliusClient.rpc.getTokenAccounts({ owner: "HAD4ZKQb4DjM6HZHemqnYFXsnYKWigMFNaP5dU951UyG", });
  const tokenAccounts = await heliusClient.connection.getAccountInfo(
    new PublicKey("CwUt9mLQNZvKtiviratTKjkMo5ZVDhJVDWAZjDoHiMgZ"),
  );

  console.log(tokenAccounts);

  console.log(`\n\x1b[1m\x1b[34m================ end ================\x1b[0m`);
})();
